describe('Fowl Prowl main-page', () => {
  ...
  it(`Should be able to click 'learn more' on a bird card and see that birds\'s details`, () => {
    cy.get('.learn-btn-card').eq(0).click()
      .url().should('include', 'learn/1')
      .get('.bird-image').eq(0).should('be.visible')
      .get('.description-block').contains('The fire-capped tit (Cephalopyrus flammiceps) is a small, 10 cm (3.9 in) long, weighing about 7 g (0.25 oz) bird species assigned to the family Paridae.')
      .get('.habitat-block').contains('Forest, woodlands and regions with solitary trees')
      .get('.fact-block').contains('This tit is able to open rolled-up leaves with its beak as starlings do, and hold it with its foot.')
  });
